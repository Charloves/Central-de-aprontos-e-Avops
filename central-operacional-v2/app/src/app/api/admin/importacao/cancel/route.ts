...
import { cancelLegacyImportForSession, LEGACY_IMPORT_CONFIRMATION_COOKIE } from '@/lib/admin/legacy-imports';
...
export async function POST(request: Request) {
  ...
  try {
    const result = await cancelLegacyImportForSession({
      session,
    ...
    const response = NextResponse.redirect(new URL('/admin/importacao?canceled=1', request.url), { status: 303 });
...
function failure(request: Request) {
  const response = NextResponse.redirect(new URL('/admin/importacao?error=cancel', request.url), { status: 303 });
  response.headers.set('Cache-Control', 'no-store');
  return response;
}
