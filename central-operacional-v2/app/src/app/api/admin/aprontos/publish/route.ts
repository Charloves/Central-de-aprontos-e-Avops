...
import { publishBriefingForSession } from '@/lib/admin/publications';
...
export async function POST(request: Request) {
  ...
  try {
    const result = await publishBriefingForSession({
      session,
      formData: await request.formData(),
      repository: new SupabasePublicationRepository(),
    });
    if (!result.ok) return failure(request);
    return redirectNoStore(new URL('/admin/aprontos?published=1', request.url));
  } catch {
...
function failure(request: Request) {
  return redirectNoStore(new URL('/admin/aprontos?error=publish', request.url));
}
