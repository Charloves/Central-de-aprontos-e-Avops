...
import { saveAvopDraftForSession } from '@/lib/admin/publications';
...
export async function POST(request: Request) {
  ...
  try {
    const result = await saveAvopDraftForSession({
      session,
      formData: await request.formData(),
      repository: new SupabasePublicationRepository(),
    });
    if (!result.ok) return failure(request, 'save');
    return redirectNoStore(new URL(`/admin/avops/novo?id=${encodeURIComponent(result.id)}&saved=1`, request.url));
  } catch {
...
function failure(request: Request, code: string) {
  return redirectNoStore(new URL(`/admin/avops/novo?error=${code}`, request.url));
}
