...
const SOURCE_PDF_NAME_PARTS = ['H50', 'OI', '2026'];
const OUTPUT_DIR = path.join(ROOT, 'oi_fases_h50');
const ROWS_JSON = path.join(ROOT, 'oi_h50_rows.json');
const ROWS_JSON_DRIVE_NAME = 'oi_h50_rows_refinado.json';
const DRIVE_FOLDER_NAME = 'OI_H50';
const INDEX_START_PAGE = 5;
const INDEX_END_PAGE = 8;
...
function findSourcePdf() {
  ...
  if (!file) throw new Error('PDF H50 nao encontrado em Downloads.');
...
function parseIndex(text, numPages) {
  ...
  const phaseRegex = /FASE\s+[\u2013-]\s+(\d{2}[A-Z]{2}\d{2})\s+[\u2013-]\s+(.+?)\s+\((SP[A-Z]{2}-\d+)\)\s*(?:\.|\s)+(\d{1,3})/y;
...
function extractMissionCodesFromText(text, faseId) {
  ...
  for (const match of missionText.match(explicitRegex) || []) {
    if (match.startsWith(faseId) && !match.endsWith('D00')) codes.add(match);
  }

  const tableRegex = /(\d{2}[A-Z]{2}\d{2}[A-Z])\)\s+EXERC\S*\s+MISS\S*\s+((?:\d{2}\s+){0,80}\d{2})/g;
  let tableMatch;
  while ((tableMatch = tableRegex.exec(missionText))) {
...
async function splitPdf(sourcePath, entries) {
  ...
  for (const entry of entries) {
    ...
    const fileName = `${slug(`H50_${entry.subprograma}_${entry.faseId}_${entry.titulo}`)}.pdf`;
...
async function findFileInFolder(token, folderId, fileName) {
  ...
  const result = await driveJson(token, `https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name,webViewLink)&pageSize=10`);
  return result.files && result.files.length ? result.files[0] : null;
}

async function uploadOrReplacePdf(token, folderId, entry) {
  const existing = await findFileInFolder(token, folderId, entry.fileName);
  const boundary = `oi_h50_${Date.now()}_${Math.random().toString(16).slice(2)}`;
  const metadata = {
    name: entry.fileName,
...
async function uploadOrReplaceJson(token, filePath, fileName) {
  const safeName = fileName.replace(/'/g, "\\'");
  const q = encodeURIComponent(`name='${safeName}' and trashed=false`);
  const search = await driveJson(token, `https://www.googleapis.com/drive/v3/files?q=${q}&fields=files(id,name,webViewLink)&pageSize=10`);
  const existing = search.files && search.files.length ? search.files[0] : null;
  const boundary = `oi_h50_json_${Date.now()}_${Math.random().toString(16).slice(2)}`;
  const metadata = {
    name: fileName,
  ...
  const text = await response.text();
  if (!response.ok) throw new Error(`Upload ${fileName} falhou: ${response.status} ${text}`);
  return JSON.parse(text);
}

function buildRows(entries, pdfUrl) {
...
async function main() {
  ...
  const indexText = await getPdfText(pdf, INDEX_START_PAGE, INDEX_END_PAGE);
  const entries = parseIndex(indexText, pdf.numPages);
  if (!entries.length) throw new Error('Nenhuma fase encontrada no indice H50.');
  ...
  for (let index = 0; index < entries.length; index += 1) {
    const file = await uploadOrReplacePdf(token, folder.id, entries[index]);
    ...
    if ((index + 1) % 20 === 0 || index === entries.length - 1) {
      console.log(`Upload H50: ${index + 1}/${entries.length}`);
    }
  }

  const currentFullPdfUrl = 'https://drive.google.com/file/d/1rXiGydmJFXmr8tgDa4fO4SkdYA_jMuoC/view?usp=sharing';
  const rows = buildRows(entries, currentFullPdfUrl);
  fs.writeFileSync(ROWS_JSON, JSON.stringify(rows, null, 2), 'utf8');
  const rowsJsonFile = await uploadOrReplaceJson(token, ROWS_JSON, ROWS_JSON_DRIVE_NAME);
  console.log(JSON.stringify({
    ...
    rowsJson: ROWS_JSON,
    rowsJsonFileId: rowsJsonFile.id,
    rowsJsonFileUrl: rowsJsonFile.webViewLink || `https://drive.google.com/file/d/${rowsJsonFile.id}/view`
  }, null, 2));
}
